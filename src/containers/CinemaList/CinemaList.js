...
class CinemaList extends Component {
			...
			<div className='movie-desc-container'>
				...
				<div className='movie-desc'>
					<ul className='movie-desc-ul'>
						...
						<li>
							<span>
								<i class='fa fa-heart heartcls' aria-hidden='true'></i>
							</span>
							{`${movie.ratings.avgRating}%`}
						</li>
						{movie.ChildEvents[0].Genre.map((i) => (
							<li className='movie-genre-cl'>{i}</li>
						))}
	...
	createCinemasList = () => {
			...
			let cinema = (
				<div className='cinema-list-item'>
					...
					<div className='cinema-name'>
						<span>
							<i class='fa fa-heart-o heartcl' aria-hidden='true'></i>
						</span>
						{cinemas[i].VenueName}
					</div>
					<div>
							...
							{showTime[i].map((time) => {
								return (
									<Link to='/seats'>
										<li
											className='show-time'
											onClick={() => this.showSeats(cinemas[i].VenueName, time)}
										>
											{time}
										</li>
									</Link>
		...
		return <div className='cinema-list'>{cinemaList}</div>;
	};

	render() {
		return (
			<div>
				<div className='cinema-movie-detail'>
					{this.createCinemaMovieDetail()}
				</div>
...
function mapDispatchToProps(dispatch) {
	return bindActionCreators(
		{ reciveTicketDetail: reciveTicketDetail },
		dispatch
	);
}
